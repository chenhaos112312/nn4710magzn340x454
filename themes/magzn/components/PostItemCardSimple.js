...
const PostItemCardSimple = ({ post }) => {
  ...
  return (
    <div
      ...
      className='lg:mb-6 max-w-screen-3xl  mr-8 py-2 gap-y-3 flex flex-col dark:border-gray-800 '>
      ...
      {siteConfig('MAGZINE_POST_LIST_COVER') && (
          <Link
            ...
            <div className='w-full w340 h454 aspect-video overflow-hidden mb-2'>
              <LazyImage
                alt={post?.title}
                src={cover}
                style={cover ? {} : { height: '0px' }}
                className='w-full h454 aspect-video object-cover'
              />
            </div>
          </Link>
        )}
