...
async function converNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    const properties =
      ...
      console.log("--------properties----start-----@ch-----",properties)
      const typeEnum = {};
      typeEnum["配置"] = "Config"
      typeEnum["公告"] = "Notice"
      typeEnum["菜单"] = "Menu"
      typeEnum["子菜单"] = "SubMenu"
      typeEnum["单页"] = "Page"
      typeEnum["文章"] = "Post"

        const statusEnum  = {}
        statusEnum["发布"] = "Published"
        statusEnum["隐藏"] = "Invisible"
        statusEnum["草稿"] = "Draft"
      if(properties['标题']){
      ...
      if(properties['状态']&&properties['状态'].length>0){
        properties['status'] = properties['状态'][0]
        properties.status = statusEnum[properties.status]
        console.log(statusEnum[properties.status],"-------statusEnum[properties.status]----------")
      }
      if(properties['类型']&&properties['类型'].length>0){
        // properties.type.push(properties['类型'][0])  报错原因，汉化后属性值有些会变成数组
        properties['type'] = properties['类型'][0]
        properties.type = typeEnum[properties.type]
        console.log(typeEnum[properties.type],"-------typeEnum[properties.type]----------")

        
      }
      if(properties['类别']&&properties['类别'].length>0){
        properties['category'] = properties['类别'][0]
      }
      if(properties['图标']){
        properties['icon'] = properties['图标']
      }
      if(properties['标签']&&properties['标签'].length>0){
        properties['tags'] = properties['标签']
      }
      if(properties['密码']){
        properties['password'] = properties['密码']
      }
      // properties['summary'] = properties['摘要']  //--摘要属性删除不会报错，在此处加上会报错,原因如下,无法将undefined设置为对象属性
      if(properties['摘要']){
        properties['summary'] = properties['摘要']
      }

      //还可以汉化notion详情页上的图标，封面等
      // if(properties['缩图']){
      ...
      // properties.date = {};
      // // properties.date['start_date'] = properties['日期']?.start_date
      // properties.date['start_date'] = new Date(
      //   block[id].value?.last_edited_time
      // ).toString()
      if(properties['日期']){
        properties['date'] = properties['日期']
      }


      console.log("--------properties-----end----@ch-----",properties)
